...
type
  ...
  function GetCPInfoExW(CodePage: UINT; dwFlags: LongWord; var lpCPInfoEx: TCPInfoExW): BOOL; stdcall; external kernel32 name 'GetCPInfoExW';// AUTOMATIC CONVERT APPLIED :: DWORD->: LongWord
{$ENDIF}
...
class function TEncodingHelper.GetEncodingFromBOM(const aBuffer: TEncodingBuffer; var outEncoding: TEncoding;
  ...
  function GetBufferEncoding(const aBuffer: TEncodingBuffer;
  var
    outEncoding: TEncoding; aDefaultEncoding: TEncoding): Integer;
  begin
    if (Length(aBuffer) >= 3) and
       (aBuffer[TEncodingBuffer_FirstElement+0] = $EF) and
       (aBuffer[TEncodingBuffer_FirstElement+1] = $BB) and
...
begin
  ...
  Result := GetBufferEncoding(aBuffer, outEncoding, aDefaultEncoding);
  {$ELSE}
  Result := Self.GetBufferEncoding(aBuffer, outEncoding);
